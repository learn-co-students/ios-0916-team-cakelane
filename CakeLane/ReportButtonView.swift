...
import UIKit
public class ReportButtonView : NSObject {
    //// Drawing Methods
...
//        let color = UIColor(red: 0.858, green: 0.464, blue: 0.299, alpha: 1.000)
        ...
        ovalPath.stroke()
        //// Rectangle Drawing
        let rectanglePath = UIBezierPath(rect: CGRect(x: 90, y: 44, width: 21, height: 67))
        color.setFill()
        rectanglePath.fill()
        //// Oval 2 Drawing
        let oval2Path = UIBezierPath(ovalIn: CGRect(x: 90, y: 122, width: 22, height: 22))
        color.setFill()
        oval2Path.fill()
